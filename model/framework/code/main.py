...
for s in smiles_list:
    try:
        pains = check_pains(s)
        brenk = check_brenk(s)
        sim = max_tanimoto(get_fingerprint(s), known_fps)
        is_sim = 1 if sim >= TANIMOTO_THRESHOLD else 0
        mol = standardize(get_mol_from_smiles(s))
        nf = 1 if mol and mol.HasSubstructMatch(nitrofuran_query) else 0
        fq = 1 if mol and mol.HasSubstructMatch(fluoroquinolone_query) else 0
        cb = 1 if mol and mol.HasSubstructMatch(carbepenem_query) else 0
        bl = 1 if mol and mol.HasSubstructMatch(betalactam_query) else 0
        outputs.append([pains, brenk, is_sim, nf, fq, cb, bl])
    except Exception:
